...
* **原理**
我们每次 `input` 事件触发后不直接去调用接口，而是设置一个时间间隔，例如 0.5s、1s…… 在这个时间间隔内，如果再次触发了 `input` 事件，则重置这个时间间隔，**也就是说**，在这个时间间隔内再次触发事件，我们就认为用户还没有操作完毕，不执行后续操作，当这个时间间隔内没有操作了，就认为用户操作完毕了，执行后续操作。
* **编码**
